...
export async function getFollowingList (token, skip, limit) {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_API_URL}/api/v1/followers/followings-list?skip=${skip}&limit=${limit}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
        "content-type": "application/json; charset=UTF-8",
...
export async function getArtworkByArtistId(artistId) {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_API_URL}/api/v1/artwork/artworks/artist/${artistId}`,
    {
      headers: {
        "content-type": "application/json; charset=UTF-8",
      }
...
export async function getCollectionByArtistId (artistId) {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_API_URL}/api/v1/collection/collections/artist/${artistId}`,
    {
      headers: {
        "content-type": "application/json; charset=UTF-8",
      }
