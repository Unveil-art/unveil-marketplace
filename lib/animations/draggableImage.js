...
class DraggableImage {
  constructor(el) {
    ...
    for (let i = 0; i <= this.trailsTotal - 1; ++i) {
      ...
      el.src = this.DOM.innerEl.querySelector("img").src;
  ...
  render() {
    this.innerScale.previous = lerp(
      this.innerScale.previous,
      this.innerScale.current,
      this.innerScale.amt
    );
    this.outerScale.previous = lerp(
      this.outerScale.previous,
      this.outerScale.current,
      this.outerScale.amt
    );
  ...
  layout() {
    ...
    if (draggingDistance > this.threshold) {
      ...
      this.trailPosition =
        this.trailPosition < this.trailsTotal - 1 ? this.trailPosition + 1 : 0;
  ...
  showNextTrailImage() {
    ...
    gsap
      .timeline({ onComplete: () => this.resetTrail(trailEl) })
      // show the image
      .set(
        trailEl,
        {
